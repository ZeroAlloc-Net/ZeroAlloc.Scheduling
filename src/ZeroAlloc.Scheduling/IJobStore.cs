...
public interface IJobStore
{
    ...
    ValueTask MarkSucceededAsync(Guid id, DateTimeOffset? nextRunAt, string? cronExpression, int maxAttempts, CancellationToken ct);

    /// <summary>Records a failed attempt and reschedules the retry.</summary>
    ValueTask MarkFailedAsync(Guid id, int attempts, DateTimeOffset nextRetryAt, CancellationToken ct);
